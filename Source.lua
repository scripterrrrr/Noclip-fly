...
local flyNoclipConn, cube, godModeConnection, freezeConn, CloneAnchorLoop, grappleSpamConn = nil, nil, nil, nil, nil, nil
local speedInput
...
-- Агрессивная попытка экипировки и активации
pcall(function() humanoid:EquipTool(quantumCloner) end)
task.wait(0.1) -- Даем игре время на обработку экипировки
pcall(function() quantumCloner:Activate() end)

-- Ожидаем появления клона (макс. 1.5 сек)
local cloneRoot = nil
local attempts = 0
repeat
task.wait(0.15)
cloneRoot = findCloneRoot()
attempts = attempts + 1
until cloneRoot or attempts >= 10

-- Убираем предмет, чтобы избежать проблем с анти-читом
...
-- GRAPPLE HOOK SPAM ACTIVATION
local backpack = LocalPlayer:FindFirstChild("Backpack")
local grappleHook = backpack and backpack:FindFirstChild("Grapple Hook")
if grappleHook and h and not grappleSpamConn then
grappleSpamConn = RunService.Heartbeat:Connect(function()
pcall(function()
h:EquipTool(grappleHook)
...
-- GRAPPLE HOOK SPAM DEACTIVATION
if grappleSpamConn then
grappleSpamConn:Disconnect()
grappleSpamConn = nil
...
if flipActive then setFlipState(false)
else setFlipState(true)
end
updateFlipButton()
updateFreezeButtons()
end)
...
-- 1. Сначала проверяем, есть ли клон
local existingClone = findCloneRoot()
if existingClone then
-- 2. Если есть, запускаем лифт
isElevatorEnabled = true
