-- =================================================================
-- Мод-меню v2.6 (Исправленный God Mode)
...
-- ================== ЛОГИКА ФУНКЦИЙ ==================
local flyActive = false; local infJumpEnabled = false
local flyNoclipConn, cube, godModeConnection = nil, nil, nil
local speedInput 
local originalWalkSpeed = 16
local originalJumpHeight = 7.2

-- ИЗМЕНЕНИЕ 1: Полностью переписанная функция God Mode
local function updateGodMode()
    local char = LocalPlayer.Character
    local humanoid = char and char:FindFirstChildWhichIsA("Humanoid")

    -- Если God Mode должен быть включен (активен флай ИЛИ прыжок)
    if flyActive or infJumpEnabled then
        if humanoid and not godModeConnection then -- Включаем, если еще не включен
            godModeConnection = RunService.Heartbeat:Connect(function()
                -- Постоянно восстанавливаем здоровье до максимума
                if humanoid.Health < humanoid.MaxHealth then
                    humanoid.Health = humanoid.MaxHealth
                end
            end)
        end
    -- Если God Mode должен быть выключен (обе функции неактивны)
    else
        if godModeConnection then -- Выключаем, если он был включен
...
local function setFlyNoclip(state)
    -- ... (код FlyNoclip без изменений)
    local char = LocalPlayer.Character; local humanoid = char and char:FindFirstChildWhichIsA("Humanoid"); if not humanoid then return end
    if state and not flyNoclipConn then
        originalWalkSpeed = humanoid.WalkSpeed; originalJumpHeight = humanoid.JumpHeight; humanoid.WalkSpeed = 0; humanoid.JumpHeight = 0
        if cube and cube.Parent then cube:Destroy() end
        cube = Instance.new("Part", workspace); cube.Size = Vector3.new(1,1,1) * 0.7; cube.Anchored = true; cube.CanCollide = false; cube.Transparency = 0.5; cube.Material = Enum.Material.Neon; cube.Name = "FlyNoclipCube"
        if char:FindFirstChild("HumanoidRootPart") then cube.CFrame = char.HumanoidRootPart.CFrame end
        flyNoclipConn = RunService.RenderStepped:Connect(function()
            ...
            local speedMultiplier = tonumber(speedInput.Text) or 1; local BASE_SPEED = 0.02; local SPEED = speedMultiplier * BASE_SPEED; local STEP = SPEED / 4
            local root = current_char.HumanoidRootPart; local flyDir = Camera.CFrame.LookVector; local nextPos = cube.Position + flyDir * STEP
            cube.CFrame = CFrame.new(nextPos, nextPos + flyDir)
            local target = cube.Position + flyDir * SPEED; root.CFrame = CFrame.new(root.Position:Lerp(target, 0.2), root.Position + root.CFrame.LookVector); root.Velocity = Vector3.new(0,0,0)
        end)
    elseif not state and flyNoclipConn then
        humanoid.WalkSpeed = originalWalkSpeed; humanoid.JumpHeight = originalJumpHeight
        flyNoclipConn:Disconnect(); flyNoclipConn = nil
        if cube and cube.Parent then cube:Destroy() end
    end
end
-- ... (Логика Infinite Jump без изменений)
UserInputService.JumpRequest:Connect(function() if notinfJumpEnabled then return end; local char = LocalPlayer.Character; local humanoid = char and char:FindFirstChildWhichIsA("Humanoid"); if humanoid then humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end end)
...
flyBtn.MouseButton1Click:Connect(function() flyActive = not flyActive; flyBtn.Text = flyActive and "Fly+Noclip: ON" or "Fly+Noclip"; setFlyNoclip(flyActive); updateGodMode() end)
infBtn.MouseButton1Click:Connect(function() infJumpEnabled = not infJumpEnabled; infBtn.Text = infJumpEnabled and "Infinity Jump: ON" or "Infinity Jump"; updateGodMode() end)
makeDraggable(icon, icon, function() mainMenu.Visible = not mainMenu.Visible end)
LocalPlayer.CharacterAdded:Connect(function(character) task.wait(0.5); updateGodMode() end)
