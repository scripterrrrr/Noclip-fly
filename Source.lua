-- =================================================================
-- Мод-меню v2.2 (Интуитивная скорость)
...
local function setFlyNoclip(state)
    ...
    if state and not flyNoclipConn then -- ВКЛЮЧАЕМ ФЛАЙ
        ...
        flyNoclipConn = RunService.RenderStepped:Connect(function()
            ...
            -- ИЗМЕНЕНИЕ 1: Скорость теперь рассчитывается в реальном времени и интуитивно
            local speedMultiplier = tonumber(speedInput.Text) or 1
            local BASE_SPEED = 0.02 -- Это "классическая" скорость, которая теперь равна "1" в поле
...
speedInput = Instance.new("TextBox", speedInputContainer); speedInput.Size = UDim2.new(0.6, 0, 1, 0); speedInput.Position = UDim2.new(0.4, 0, 0, 0);speedInput.BackgroundColor3 = Color3.fromRGB(20, 20, 20); speedInput.BackgroundTransparency = 0.5; speedInput.TextColor3 = Color3.new(1, 1, 1); speedInput.Font = Enum.Font.SourceSansBold
speedInput.TextSize = 18
-- ИЗМЕНЕНИЕ 2: Значение по умолчанию теперь "1"
speedInput.Text = "1"
speedInput.ClearTextOnFocus = false; Instance.new("UICorner", speedInput).CornerRadius = UDim.new(0, 4)
